...
Payment.prototype.initEvents = function () {
  ...
  function onPageLoad() {
    callServerAPI("GET", RTLServerURL + CONSTANTS.RTL_CONF_URL, serverToken)
        ...
        if (self.loadedFrom !== "ERROR") {
          setTimeout(function () {
            $(document).on("click", ".node-list-block", function () {
              $("#selectNodeRadioInput" + $(this).attr("id").substring(5)).prop(
                "checked",
                true
              );
          ...
          $("#selectNodeRadioInput" + rtlConfig.selectedNodeIndex).prop(
            "checked",
            true
          ); //Do Not Remove: It sets request options from get_info
          $("#selectNodeRadio").trigger("change"); //Do Not Remove: It sets request options from get_info
        } else {
          $("#invoice").val(
            "Either fix the connection for selected node or select another node to pay"
          );
      ...
      .catch((err) => {
        loadModule({
          load: CONSTANTS.MODULES.ERROR,
          loadedFrom: CONSTANTS.MODULES.PAYMENT,
  ...
  $("#sendPaymentBtn").click(function () {
    ...
    switch (selectNodeImplementation) {
      case "CLN":
        if (
          !self.decodedPaymentResponse.msatoshi &&
          invoiceAmount &&
          invoiceAmount > 0
        ) {
          reqData = {
            paymentType: "INVOICE",
            invoice: invoiceVal,
            amount: parseInt(invoiceAmount * 1000),
          };
        } else {
      ...
      case "ECL":
        if (
          !self.decodedPaymentResponse.amount &&
          invoiceAmount &&
          invoiceAmount > 0
        ) {
          reqData = {
            invoice: invoiceVal,
            amountMsat: parseInt(invoiceAmount * 1000),
          };
        } else {
      ...
      default:
        if (
          !self.decodedPaymentResponse.num_satoshis &&
          invoiceAmount &&
          invoiceAmount > 0
        ) {
    ...
    callServerAPI("POST", final_url, serverToken, JSON.stringify(reqData))
      .then((data) => {
        loadModule({
          load: CONSTANTS.MODULES.STATUS,
          loadedFrom: CONSTANTS.MODULES.PAYMENT,
          status: "SUCCESS",
          title: "Payment status:",
          message: createPaymentStatusHTML(
            "SUCCESS",
            selectNodeImplementation,
            data.paymentResponse ? data.paymentResponse : data
          ),
        });
  ...
  $("#selectNodeRadio").on("change", function () {
    if (rtlConfig && rtlConfig.nodes.length > 0) {
      ...
      if (rtlConfig.nodes.length == 1) {
        ...
        selectNodeImplementation = rtlConfig.nodes[0].lnImplementation
          ? rtlConfig.nodes[0].lnImplementation
          : "LND";
        switch (selectNodeImplementation) {
          case "CLN":
        ...
        newSelNodeIndex = +$(
          "input[name='selectNodeRadioInput']:checked"
        ).val();
        filteredNode = rtlConfig.nodes.filter((node) => {
          return node.index == newSelNodeIndex;
        })[0];
        selectNodeImplementation =
          filteredNode.lnImplementation && filteredNode.lnImplementation != ""
            ? filteredNode.lnImplementation.toUpperCase()
            : "LND";
        callServerAPI(
          "POST",
          RTLServerURL + CONSTANTS.UPDATE_SEL_NODE_URL,
          serverToken,
          JSON.stringify({
            currNodeIndex: newSelNodeIndex,
            prevNodeIndex: selNode.index,
          })
        )
          .then((selNodeResponse) => {
            selNode = rtlConfig.nodes.filter(
              (node) => node.index == newSelNodeIndex
            )[0];
  ...
  function updateStyles(selNodeIndex) {
    browser.storage.sync.get("SERVER_CONFIG", function (storage) {
      if (storage.SERVER_CONFIG && storage.SERVER_CONFIG.nodes) {
        ...
        let selNode = storage.SERVER_CONFIG.nodes.filter(
          (node) => node.index == selNodeIndex
        )[0];
        if (
          selNode &&
          selNode.settings &&
          selNode.settings.themeMode &&
          selNode.settings.themeColor
        ) {
          $('link[id="themeStyle"]').attr(
            "href",
            "./styles/" +
              selNode.settings.themeMode.toLowerCase() +
              "/" +
              selNode.settings.themeColor.toLowerCase() +
              ".css"
          );
        }
      }
    ...
    if (
      ...
      switch (selectNodeImplementation) {
        case "CLN":
          final_url =
            RTLServerURL +
            CONSTANTS.API_URL.CLN.GET_PAYMENT_DETAILS +
            "/" +
            invoiceVal;
          break;
        case "ECL":
          final_url =
            RTLServerURL +
            CONSTANTS.API_URL.ECL.GET_PAYMENT_DETAILS +
            "/" +
            invoiceVal;
          break;
        default:
          final_url =
            RTLServerURL +
            CONSTANTS.API_URL.LND.GET_PAYMENT_DETAILS +
            "/" +
            invoiceVal;
          break;
      }
      callServerAPI("GET", final_url, serverToken)
        .then((paymentDetailsResponse) => {
          ...
          if (
            ...
            (selectNodeImplementation == "CLN" &&
              !paymentDetailsResponse.msatoshi) ||
            (selectNodeImplementation == "ECL" &&
              !paymentDetailsResponse.amount)
          ) {
            $("#invoiceAmount").focus();
        ...
        .catch((err) => {
          // $('#paymentDetails').html('Invalid ID, Please Enter Correct Invoice ID');
          $("#paymentDetails")
  ...
  $("#paymentDetails").on("keyup", "#invoiceAmount", function (event) {
    ...
    if (
      invoiceAmt &&
      invoiceAmt > 0 &&
      CONSTANTS.NUMBER_REGEX.test(invoiceAmt)
    ) {
      $("#sendPaymentBtn").removeAttr("disabled");
    } else {
      $("#sendPaymentBtn").attr("disabled", true);
    ...
    if (rtlConfig.nodes.length) {
      if (rtlConfig.nodes.length == 1) {
        str =
          ...
          '<input type="radio" class="custom-control-input" name="selectNodeRadioInput" id="selectNodeRadioInput' +
          rtlConfig.nodes[0].index +
          '" value="' +
          rtlConfig.nodes[0].index +
          '" checked="">' +
          '<label class="custom-control-label" for="selectNodeRadioInput' +
          rtlConfig.nodes[0].index +
          '">' +
          rtlConfig.nodes[0].lnNode +
          "(" +
          rtlConfig.nodes[0].lnImplementation +
          ")" +
          "</label>" +
        ...
        $("#selectNodeRadioInput" + rtlConfig.nodes[0].index).prop(
          "checked",
          true
        );
        selectNodeImplementation = rtlConfig.nodes[0].lnImplementation
        ...
        for (var i = 0; i < rtlConfig.nodes.length; i++) {
          str =
            ...
            rtlConfig.nodes[i].index +
            '">' +
            rtlConfig.nodes[i].lnNode +
            "(" +
            rtlConfig.nodes[i].lnImplementation +
            ")" +
            "</label>" +
            "</div>";
          selectNodeRadio.append(str);
          if (rtlConfig.nodes[i].index == rtlConfig.selectedNodeIndex) {
            ...
            selectNodeImplementation =
              rtlConfig.nodes[i].lnImplementation &&
              rtlConfig.nodes[i].lnImplementation != ""
                ? rtlConfig.nodes[i].lnImplementation.toUpperCase()
                : "LND";
          }
        }
      }
  ...
  function createPaymentDetailsHTML(
    status,
    selectNodeImplementation,
    paymentDetailsResponse
  ) {
    ...
    if (status == "SUCCESS") {
      switch (selectNodeImplementation) {
        case "CLN":
          if (paymentDetailsResponse.payee) {
            var expiryDate = new Date(
              (+paymentDetailsResponse.created_at +
                +paymentDetailsResponse.expiry) *
                1000
            );
            var expiryDateStr =
              ("0" + expiryDate.getDay()).slice(-2) +
              "/" +
              CONSTANTS.MONTHS[expiryDate.getMonth()].name +
              "/" +
            ...
            details_html =
              '<div class="row"><div class="col-3"><p class="my-0">Destination: </p></div><div class="col-9"><p class="my-0">' +
            ...
            if (paymentDetailsResponse.msatoshi) {
              ...
              details_html =
                details_html +
                '<input type="text" class="form-control col-6 invoice-amount mb-2" id="invoiceAmount" placeholder="Invoice amount" tabindex="9">';
            }
            details_html =
              details_html +
              '<span class="col-6 pl-1"> Sats</span></p></div></div><hr><div class="row"><div class="col-3"><p class="my-0">Expiry: </p></div><div class="col-9"><p class="my-0">' +
              expiryDateStr +
              "</p></div></div>";
          }
          break;
        case "ECL":
          if (paymentDetailsResponse.nodeId) {
            var expiryDate = new Date(
              (paymentDetailsResponse.timestamp +
                (paymentDetailsResponse.expiry
                  ? paymentDetailsResponse.expiry
                  : 3600)) *
                1000
            ...
            if (paymentDetailsResponse.amount) {
              details_html =
                details_html +
                formatNumberWithCommas(parseInt(paymentDetailsResponse.amount));
            } else {
              details_html =
                details_html +
                '<input type="text" class="form-control col-6 invoice-amount mb-2" id="invoiceAmount" placeholder="Invoice amount" tabindex="9">';
        ...
        default:
          if (paymentDetailsResponse.destination) {
            ...
            if (
              paymentDetailsResponse.num_satoshis &&
              +paymentDetailsResponse.num_satoshis > 0
            ) {
              details_html =
                details_html +
                formatNumberWithCommas(+paymentDetailsResponse.num_satoshis);
            } else {
              details_html =
                details_html +
                '<input type="text" class="form-control col-6 invoice-amount mb-2" id="invoiceAmount" placeholder="Invoice amount" tabindex="9">';
      ...
      switch (selectNodeImplementation) {
        case "CLN":
          return (
            '<div class="row px-2"><div class="col-3"><p class="my-0">Error: </p></div><div class="col-9"><p class="my-0">' +
        ...
        default:
          return (
            ...
            '</p></div></div><hr><div class="row px-2"><div class="col-3"><p class="my-0">Description: </p></div><div class="col-9"><p class="my-0">' +
            (paymentDetailsResponse.error.error
              ? paymentDetailsResponse.error.error
              : paymentDetailsResponse.error.errno) +
            "</p></div></div>"
          );
      }
    }
  ...
  function createPaymentStatusHTML(
    status,
    selectNodeImplementation,
    ...
    if (status == "SUCCESS") {
      switch (selectNodeImplementation) {
        case "CLN":
          let fee = 0;
          if (
            paymentStatusResponse.msatoshi_sent &&
            paymentStatusResponse.msatoshi
          ) {
            fee =
              paymentStatusResponse.msatoshi_sent -
              paymentStatusResponse.msatoshi;
          ...
          return (
            '<div class="row"><div class="col-3"><p class="my-0">Preimage: </p></div><div class="col-9"><p class="my-0">' +
        ...
        case "ECL":
          return (
            '<div class="row"><div class="col-4"><p class="my-0">Submitted with ID: </p></div><div class="col-8"><p class="my-0">' +
            paymentStatusResponse +
            "</p></div></div><hr>"
          );
        default:
        ...
        default:
          return (
            ...
            (paymentStatusResponse.error.code
              ...
              : paymentStatusResponse.error) +
            '</p></div></div><hr><div class="row px-2"><div class="col-3"><p class="my-0">Description: </p></div><div class="col-9"><p class="my-0">' +
            (paymentStatusResponse.error.error
              ? paymentStatusResponse.error.error
              : paymentStatusResponse.error) +
            "</p></div></div>"
          );
